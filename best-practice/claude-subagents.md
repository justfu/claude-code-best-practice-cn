...
Claude Code Sub-agents — frontmatter 字段和官方内置 Agent 类型。
...
| `tools` | string/list | 否 | 逗号分隔的工具白名单（如 `Read, Write, Edit, Bash`）。省略时继承所有工具。支持 `Agent(agent_type)` 语法限制可生成的 subagent；旧的 `Task(agent_type)` 别名仍然有效 |
...
| `maxTurns` | integer | 否 | subagent 停止前的最大 agentic 轮次数 |
| `skills` | list | 否 | 启动时预加载到 agent 上下文中的 Skill 名称（注入完整内容，而非仅使其可用） |
| `mcpServers` | list | 否 | 此 subagent 的 MCP 服务器 — 服务器名称字符串或内联 `{name: config}` 对象 |
| `hooks` | object | 否 | 限定于此 subagent 的生命周期 hooks。所有 hook 事件都支持；`PreToolUse`、`PostToolUse` 和 `Stop` 最常用 |
| `memory` | string | 否 | 持久化 memory 作用域：`user`、`project` 或 `local` |
| `background` | boolean | 否 | 设为 `true` 始终作为后台任务运行（默认：`false`） |
| `effort` | string | 否 | 此 subagent 激活时的 effort 级别覆盖：`low`、`medium`、`high`、`max`（仅限 Opus 4.6）。默认：继承自 session |
| `isolation` | string | 否 | 设为 `"worktree"` 在临时 git worktree 中运行（无更改时自动清理） |
| `initialPrompt` | string | 否 | 当此 agent 作为主 session agent 运行时（通过 `--agent` 或 `agent` 设置），自动作为第一个用户轮次提交。Commands 和 Skills 会被处理。附加到任何用户提供的提示之前 |
...
| # | Agent | 模型 | 工具 | 描述 |
|---|-------|-------|-------|-------------|
| 1 | `general-purpose` | inherit | 全部 | 复杂多步骤任务 — 研究、代码搜索和自主工作的默认 agent 类型 |
| 2 | `Explore` | haiku | 只读（无 Write、Edit） | 快速代码库搜索和探索 — 优化用于查找文件、搜索代码和回答代码库问题 |
| 3 | `Plan` | inherit | 只读（无 Write、Edit） | Plan 模式中的预规划研究 — 在编写代码前探索代码库并设计实现方案 |
